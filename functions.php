...
define( 'CHILD_THEME_NAME', 'Local Life' );
...
add_action( 'genesis_meta', 'locallife_viewport_meta_tag' );
function locallife_viewport_meta_tag() {
	echo '<meta name="viewport" content="width=device-width, initial-scale=1.0"/>';
}

/** Create additional color style options (default is Red) */
add_theme_support( 'genesis-style-selector', array(
	'local-life-blue'	=> __( 'Blue', 'Local Life' ),
	'local-life-gray'	=> __( 'Gray', 'Local Life' )
) );
...
add_theme_support( 'genesis-custom-header', array( 'width' => 1152, 'height' => 150 ) );
...
genesis_register_sidebar( array(
	'id'			=> 'home-slider',
	'name'			=> __( 'Home Slider', 'locallife' ),
	'description'	=> __( 'The slider section of the homepage.', 'locallife' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'home-top-right',
	'name'			=> __( 'Home Top Right', 'locallife' ),
	'description'	=> __( 'The top right section of the homepage (next to slider).', 'locallife' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'home-middle',
	'name'			=> __( 'Home Middle', 'locallife' ),
	'description'	=> __( 'The responsive middle section of featured items on the homepage.', 'locallife' ),
) );
...
function my_custom_login_logo() {
	...
	<style type="text/css"><!-- body.login #login h1 a { background-image:url('.get_bloginfo('template_directory').'/images/bgrlocal-login.png) !important; background-size: 340px 90px; width: 320px; height: 90px; }--></style>';
}
add_action('login_head', 'my_custom_login_logo');


add_filter( 'genesis_author_box', 'bgrl_author_box', 10, 6 );
/**
 * Custom Author Box for BGRLocal
 *
 * @param string $output
 ...
 */
function bgrl_author_box( $output, $context, $pattern, $gravatar, $title, $description ) {
...
add_filter( 'display_posts_shortcode_output', 'be_display_posts_add_meta_readmore', 10, 7 );
function be_display_posts_add_meta_readmore( $output, $atts, $image, $title, $date, $excerpt, $inner_wrapper ) {
